...
def PMCMC(iterations, num_particles, init_theta, prior, model, data, rng, dt,model_dim): 
    ...
    for iter in range(1,iterations): 
        ...
        # if iter > burn_in:
        #     theta_prop = rng.multivariate_normal(theta[:, iter-1],cov = scale * 2.38/np.sqrt(len(init_theta)) * np.cov(theta))

        # else:  
        #     theta_prop = rng.multivariate_normal(theta[:, iter-1],cov = scale * np.eye(np.shape(theta)[0]))
